# Govee LAN Control for Home Assistant (2026.3+ Fix)

[![hacs_badge](https://img.shields.io/badge/HACS-Custom-41BDF5.svg?style=for-the-badge)](https://github.com/hacs/integration)

[![Open your Home Assistant instance and open a repository inside the Home Assistant Community Store.](https://my.home-assistant.io/badges/hacs_repository.svg)](https://my.home-assistant.io/redirect/hacs_repository/?owner=Blacktime91&repository=New-2026-Govee-LAN-Control&category=integration)

## About this fork

This is a maintained fork of [wez/govee-lan-hass](https://github.com/wez/govee-lan-hass) (v1.0.0), which is no longer actively maintained. The original integration broke after the Home Assistant 2026.3.0 update due to [removed deprecated light constants](https://developers.home-assistant.io/blog/2026/02/23/remove-deprecate-light-features).

### Changes from the original (v1.1.0)

All changes are in `custom_components/govee_lan/light.py`:

- **Removed deleted imports:** `ATTR_COLOR_TEMP`, `SUPPORT_BRIGHTNESS`, `SUPPORT_COLOR`, `SUPPORT_COLOR_TEMP` (removed from HA 2026.3)
- **Removed invalid color mode:** `ColorMode.BRIGHTNESS` from `supported_color_modes` (invalid in combination with `COLOR_TEMP`/`RGB`, now raises a hard error)
- **Removed mired attributes:** `_attr_color_temp` assignments (attribute removed from `LightEntity`)
- **Removed mired service handler:** `ATTR_COLOR_TEMP` fallback in `async_turn_on()` (HA only sends `ATTR_COLOR_TEMP_KELVIN` now)
- **Removed unused import:** `homeassistant.util.color` (no longer needed without mired conversions)

For full details, see the [HA 2026.3 release notes](https://www.home-assistant.io/blog/2026/03/04/release-20263/#backward-incompatible-changes).
...
3. Add `https://github.com/Blacktime91/New-2026-Govee-LAN-Control` with category **Integration**
4. Search for "Govee LAN Control" and install it
5. Restart Home Assistant
6. Go to Settings → Devices & Services → **Add Integration**
7. Search for "Govee LAN Control" and enter your HTTP API key

> **Important:** If you previously used the original `wez/govee-lan-hass` integration, remove it from HACS first (the HA integration config can stay).
...
Original integration by [@wez](https://github.com/wez) — [govee-lan-hass](https://github.com/wez/govee-lan-hass). HA 2026.3 compatibility fix by [@Blacktime91](https://github.com/Blacktime91).
